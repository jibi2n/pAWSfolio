...
const panel = 'absolute right-0 top-14 w-64 p-2 bg-card rounded-2xl shadow-[0_16px_40px_rgba(46,26,95,0.18)] whitespace-normal'
...
export function Nav({ user }: { user: PublicUser | null }) {
  ...
  return (
    <nav className="fixed z-50 top-3 inset-x-3 lg:inset-x-auto lg:top-4 lg:left-1/2 lg:-translate-x-1/2 lg:min-w-[720px] lg:max-w-[900px] shadow-purple bg-card rounded-full pl-2 pr-2 lg:px-4 py-2 flex items-center gap-2 whitespace-nowrap">
        ...
        {links.map(({ href, label, isActive }) => {
          ...
          return (
            <Link
              ...
              className={`nav-item px-4 py-2 rounded-full text-[14px] font-medium ${active ? 'active' : 'text-text hover:bg-hover'}`}
            >
              {label}
            </Link>
          )
      ...
      {user ? (
        ...
        <div className="hidden lg:flex items-center gap-2">
          <Link href="/login" className="px-4 py-2 rounded-full text-[14px] font-medium text-text hover:bg-hover">Log in</Link>
      ...
      <details key={`m${pathname}`} className="relative lg:hidden">
        <summary className="list-none cursor-pointer flex items-center gap-2 rounded-full p-1" aria-label="Menu">
          ...
          <span className="w-10 h-10 rounded-full flex items-center justify-center bg-lavender-bg text-on-soft">
          ...
          {links.map(({ href, label, isActive }) => {
            ...
            return (
              <Link
                ...
                href={href}
                aria-current={active ? 'page' : undefined}
                className={`${menuRow} font-medium ${active ? 'bg-mint text-ink' : 'text-text hover:bg-hover'}`}
              >
                {label}
              </Link>
            )
