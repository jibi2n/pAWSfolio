...
export function BuildDetail({ build, user, isNew }: { build: Build; user: PublicUser | null; isNew: boolean }) {
  return (
    <div className="max-w-[800px] mx-auto px-5 sm:px-8 pt-24 sm:pt-28 pb-20">
      {isNew && (
        <div className="flex items-center gap-4 p-5 mb-6 rounded-card bg-card shadow-purple" role="status">
          <MascotCloud size={64} />
          <div>
            <p className="text-[18px] font-semibold text-text">Your build is live! ☁️</p>
            <p className="text-[14px] text-subtle">Time to share it with the world.</p>
          </div>
        </div>
      )}

      <div className="flex items-center justify-between mb-6">
        <Link href="/" className="flex items-center gap-2 text-[14px] text-link font-medium hover:opacity-80">
      ...
      {build.link && (
        <a href={build.link} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-4 rounded-[16px] mb-6 bg-card border border-lavender-bg hover:opacity-80 transition-opacity">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[14px] text-link font-medium truncate">{build.link}</div>
          </div>
